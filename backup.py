...
def find_solution(n, timer):
    ...
    for position in positions:
        # Don't spend time running "is_possible" function if the position is already assigned.
        if position not in assignments.values():
        ...
        for pair in lst:
            for n2 in range(2):
                if student == pair[n2%2] and pair[(n2+1)%2] in assignments:
                    # Compatibles too far
                    if n1 == 0 and (not are_neighboors_x(position, assignments[pair[(n2+1)%2]])):
                        return False
...
def are_neighboors(position, partner_position):
    for n in range(2):
...
def are_neighboors_x(position, partner_position):
